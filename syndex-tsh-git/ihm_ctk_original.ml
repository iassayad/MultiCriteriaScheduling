...
(**********************************)
  ...
  and fault_tolerance_adequation adeqtype =
    ...
    let check,npf,nmf = true,1,0 in                     
             ...
             Fault_tolerance_adequation.failures := {Fault_tolerance_adequation.npf=npf; Fault_tolerance_adequation.nmf=nmf};
	     ...
	     schedule_view Fault_tolerance_adequation.pretty_conv_graph
...
(********************************************************************) 
  and simulation x = 
  let dir = "/usr/local/kalla/Reliability/" in  
  ...
  let out_readme = open_out readme in
    ...
    for ind_p = 0 to 0 do (* architecture size = tab_p[ind_p] *) 
      ...
      let ccr = ref 0. in 
      let tab_ccr = [|0.1;1.0;10.|] in 
	for ind_ccr = 0 to 0 do  
	  ccr := tab_ccr.(ind_ccr);  
	  let ftbar = (dir^"results/len-ftbar1-"^(string_of_float !ccr)) in 
          let out_ftbar = open_out ftbar in   
	    for i = 1 to 1 do  (* task graphes size = i*10 *) 
 	      let size =  i * 20 in
                output_string out_ftbar (string_of_int size);  
                let dir_opr=(dir^"graphs/p"^(string_of_int !p)^"/t"^(string_of_int size)^"/")in
                for j = 1 to 3 do (*nb red graphs*) 
	  	  let algo_name = ("alg"^(string_of_int i)^"_red"^(string_of_int j)) in
                  pss "-----------------------------------------------";
		  ...
		  open_file_name_simulation name; 
                  ...
                  output_string out_ftbar (" "^(string_of_float !Fault_tolerance_adequation.len));
                  pss (" FTBAR  = "^(string_of_float  !Fault_tolerance_adequation.len));
                  let ftbar_len = !Fault_tolerance_adequation.len in                   
                    flush out_ftbar;
                    pss "";  
                    close_file (); 
                done;
	        output_string out_ftbar "\n";
	    done;   
	    close_out out_ftbar;
	done;
    done
...
(*   pack [debug_button] [Side Side_Left; Fill Fill_Y];  *)
  ...
  (*let name_sdx = ("/usr/local/kalla/Reliability/v6/DSN.sdx") in
  let name_sdx = ("/usr/local/kalla/Reliability/graphs/p5/t20/alg1_red2_ccr1._archi_p5.sdx") in
   open_file_name name_sdx;
  let name_rel = ("/usr/local/kalla/Reliability/v6/DSN.rel") in
  let name_rel = ("/usr/local/kalla/Reliability/graphs/p5/t20/alg1_red2_ccr1._taux1._archi_p5.rel") in
   reliability_adequation No_Flatten name_rel*)
