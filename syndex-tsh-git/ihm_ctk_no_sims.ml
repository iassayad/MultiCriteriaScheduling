...
(*       [],"F7", (function _ -> fault_tolerance_adequation No_Flatten); *)
  ...
  and fault_tolerance_adequation adeqtype = 
    (match Xsc_ctk.faults_data_choose top with
       ...
       | true,npf,nmf,check ->  (
     Schedule_ctk.reliability_cost := "";
     (*let check,npf,nmf = true,npf,nmf in                        *)
...
(*  and fault_tolerance_generate_code chrono =
  ...
  (******** RELIABILITY     ***********)
  (************************************)
  and reliability_adequation adeqtype =
    (match Xsc_ctk.reliability_data_choose top with
       | false,_,_,_,_    -> () 
       | true,obj_len,obj_rel,level_max,theta ->  
       Reliability_adequation.obj_rel := obj_rel;
       Reliability_adequation.obj_len := obj_len;
       Reliability_adequation.theta := theta;
       Reliability_adequation.level_max := level_max;
       Reliability_adequation.file_name := !syndex_file_name;     
       adequation adeqtype (Reliability_adequation.reliability_adequation);
       Schedule_ctk.reliability_cost := (string_of_float !Reliability_adequation.old_rel))
...
(*   Menu.add_command constraints_menu [Label "Relative Constraints"; Command (function () -> Xsc_ctk.relative_constraints top)]; *)
 ...
 (**************** FAULT TOLERANCE MENU ***************)
  Menu.add_command adeq_menu [Label "Fault tolerance adequation";
                   Command(function _ -> fault_tolerance_adequation No_Flatten)]; 
 (**************** FAULT TOLERANCE MENU ***************)
  Menu.add_command adeq_menu [Label "Reliability adequation"; 
                  Command(function _ -> 
                  reliability_adequation No_Flatten)]; 
  Menu.add_separator adeq_menu; 
 (************************ END ************************) 
