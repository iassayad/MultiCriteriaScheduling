...
module Tsh_adequation = Tsh_adequation.Make (Progress_box.Tk)
...
(**********************************)
 ...
 (* and choose_lib_dir () =
  ...
  and tsh_adequation adeqtype = 
    let name_sdx,name_file_rel,name_file_frq = 
 (* "/home/popart/assayad/tricriteres/syndex-tsh/file-sdx/sampp-3p-30o.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/4-proc-heterogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/4-proc-heterogene-media-10e5.frq"
*)
  (*    "/home/popart/assayad/tricriteres/syndex-tsh/file-sdx/sammp-3p-9o.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/4-proc-heterogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/4-proc-heterogene-media-10e5.frq"
*)
   (* "/home/popart/assayad/tricriteres/syndex-tsh/file-sdx/sampp-4p-30o.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/4-proc-heterogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/4-proc-heterogene-media-10e5.frq"
 *)
(*     "/home/popart/assayad/tricriteres/syndex-tsh/file-sdx/sampp-5p-30o.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/5-proc-homogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/5-proc-homogene-media-10e5.frq"   
 *)
 (*   "/home/popart/assayad/tricriteres/syndex-tsh/file-sdx/sampp-6p-6o.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/6-proc-heterogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/6-proc-homogene-media-10e5.frq" 
*)
  (*  "/home/popart/assayad/tricriteres/syndex-tsh/file-sdx/sampp-6p-30o.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/6-proc-heterogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/6-proc-homogene-media-10e5.frq" 
 *)
   "/home/popart/assayad/app-sorel-syndexV6.sdx", 
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/6-proc-heterogene-media-10e5.rel",
      "/home/popart/assayad/tricriteres/syndex-tsh/file-rel/6-proc-homogene-media-10e5.frq" 

in
    open_file_name_simulation name_sdx;
    adequation No_Flatten (Latency_adequation.adequation name_file_rel);
           let max_replication =  3 in
           let nbproc = 6 in
           let nbop = 30 in
  (*  let out_channel1 = open_out_gen [Open_create] 666 "big_lambda.txt"  in
    let out_channel2 = open_out_gen [Open_create] 666 "power.txt" in
    let out_channel3 = open_out_gen [Open_create] 666 "length.txt" in
    *)
    let out_channel1 = open_out ("../tsh-results/big_lambda_rep"^string_of_int(max_replication)^"_nbproc"^string_of_int(nbproc)^"_nbop"^string_of_int(nbop)^".txt")  in
    let out_channel2 = open_out ("../tsh-results/power_rep"^string_of_int(max_replication)^"_nbproc"^string_of_int(nbproc)^"_nbop"^string_of_int(nbop)^".txt") in
    let out_channel3 = open_out ("../tsh-results/length_rep"^string_of_int(max_replication)^"_nbproc"^string_of_int(nbproc)^"_nbop"^string_of_int(nbop)^".txt") in
    let out_channel4 = open_out ("../tsh-results/big_lambda_obj"^string_of_int(max_replication)^"_nbproc"^string_of_int(nbproc)^"_nbop"^string_of_int(nbop)^".txt")  in
    let out_channel5 = open_out ("../tsh-results/power_obj"^string_of_int(max_replication)^"_nbproc"^string_of_int(nbproc)^"_nbop"^string_of_int(nbop)^".txt") in
    for l2 = 1 to 20 do
     for l1 = 1 to 2 do 
      for p2 = 1 to 40 do
...
*)
       ...
       let obj_lambda = (float_of_int(l1)) *. 4. *. (10. **(-. (21.-.float_of_int(l2)))) in  

    
       adequation No_Flatten (Reliability_adequation.adequation !Latency_adequation.schedule_length
			     obj_lambda 45. max_replication false name_file_rel);
    
    (*  let pui_max = (!Reliability_adequation.exe_opns /. !Reliability_adequation.schedule_length) in *)
      let pui_max = 4. in 
      let obj_pui = (float_of_int(p2)) *. 0.05 *. pui_max in
    
      adequation No_Flatten (Tsh_adequation.adequation obj_lambda obj_pui max_replication name_file_rel name_file_frq);

      ps ("\n\n obj_lam = "^(string_of_float obj_lambda)^
	"\n obj_pui = "^(string_of_float obj_pui)
        ^"\n---------\n---------\n len_tsh = "^(string_of_float !Tsh_adequation.schedule_length)
        ^"\n lam_tsh = "^(string_of_float !Tsh_adequation.schedule_big_lambda)
        ^"\n pui_tsh = "^(string_of_float (!Tsh_adequation.old_pui/. !Tsh_adequation.schedule_length))
	^"\n rep_tsh = "^(string_of_float !Tsh_adequation.rate_replication)^"\n\n");   
	
	(* tsh_schedule_view (); *)	
	match !Tsh_adequation.schedule_failure  with
	   | false ->	
     		output_string out_channel1 ((string_of_float !Tsh_adequation.schedule_big_lambda)^" ");
		output_string out_channel2 ((string_of_float (!Tsh_adequation.old_pui/. !Tsh_adequation.schedule_length))^" "); 
                (* output_string out_channel2 ((string_of_float (!Tsh_adequation.old_pui)^" ")); *)
                output_string out_channel4 ((string_of_float obj_lambda)^" ");
      		output_string out_channel5 ((string_of_float obj_pui)^" ");
                output_string out_channel3 ((string_of_float !Tsh_adequation.schedule_length)^" ");
	   | true  ->
      		 output_string out_channel1  "NaN ";
      		 output_string out_channel2  "NaN ";
      		 output_string out_channel3 "Nan ";
                 output_string out_channel4 ((string_of_float obj_lambda)^" ");
      		 output_string out_channel5 ((string_of_float obj_pui)^" ");
      done; 
      output_string out_channel1 ("\n");
      output_string out_channel2 ("\n");
      output_string out_channel3 ("\n");
      output_string out_channel4 ("\n");
      output_string out_channel5 ("\n");
     done; 
    done; 

   close_out out_channel1;
   close_out out_channel2;
   close_out out_channel3;
   close_out out_channel4;
   close_out out_channel5;

and tsh_schedule_view () =
 schedule_view Adequation_core.pretty_conv_graph "SYNDEX"
...
and simulation_ccr () = 
    let what_sim = "TSH" in
    let lambda_e = "5" in 
    let dir = ("/scratch/kalla/Benchmarks/Results/"^what_sim^"/media-10e"^lambda_e^"/")  in   
    let dir_graphs = "/scratch/kalla/Benchmarks/" in 
    let dir_rel    = "/scratch/kalla/Benchmarks/SynDEx-TSH/file-rel/" in  
    let readme = dir^"SimParameters" in 
    let out_readme = open_out readme in
    output_string out_readme ("\n\n AAA-TSF :\n\n");  
    output_string out_readme ("\n\n TSH :\n\n");  
    output_string out_readme (" p = 5  \n ccr = [1.0] \n N = 50 (x50graphs) \n  rep max = 3 (donc 4)");  
    close_out out_readme;   
    let p = ref 0 in  
    let tab_p = [|5|] in  (* NUMBER OF PROCESSORS *) 
    for ind_p = 0 to 0 do  
      p := tab_p.(ind_p);        
	...
	for i = 5 to 5 do  
	  ...
	  output_string out_aaa_lam (string_of_int size); output_string out_aaaf_lam (string_of_int size);    
          let dir_opr=(dir_graphs^"Random_graphs/p"^(string_of_int !p)^"/t"^(string_of_int size)^"/")in                
	  ...
	  let nb_red = 1 in
	    ...
	    let name_rel = (dir_rel^(string_of_int !p)^"-proc-homogene-media-10e"^lambda_e^".rel") in
	    pss name;                           
	      ...
	      (*output_string out_aaa_len  (" "^(string_of_float !Latency_adequation.schedule_length));  
	       ...
	       pss ("\n AAA-F-ok-routing("^(string_of_float (!Reliability_adequation.rate_replication)) 
		    ...
		    ^"     /\\*    = "^(string_of_float  lambda) 
               ...
               adequation No_Flatten (Reliability_adequation.adequation max_float lambda 45. 0 false name_rel);
	       output_string out_aaa_len  (" "^(string_of_float !Reliability_adequation.schedule_length)); 
	       output_string out_aaa_lam  (" "^(string_of_float !Reliability_adequation.schedule_big_lambda)); 

	      pss ("\n AAA-F("^(string_of_float (!Reliability_adequation.rate_replication))
                   ^")      L     = "^(string_of_float  !Reliability_adequation.schedule_length)
		   ^"     /\\*    = "^(string_of_float  lambda)
                   ^"     /\\     = "^(string_of_float  !Reliability_adequation.schedule_big_lambda)^"%");
...
(*   Menu.add_command constraints_menu [Label "Relative Constraints"; Command (function () -> Xsc_ctk.relative_constraints top)]; *)
 ...
 (**************** FAULT TOLERANCE MENU ***************)
  Menu.add_command adeq_menu [Label "Fault-Tolerance AAA-TB/TP";Accelerator "F7";Command(function _ -> fault_tolerance_adequation No_Flatten)];
  Menu.add_command adeq_menu [Label "TSH";Accelerator "F11";Command(function _ -> tsh_adequation No_Flatten)];
  Menu.add_separator adeq_menu; 
 (************************ END ************************) 

   Menu.add_command adeq_menu [Label "View schedule"; Accelerator "F8"; Command (function _ -> latency_adequation_schedule_view ())];

(* Menu.add_command adeq_menu [Label "View schedule"; Accelerator "F8"; Command (function _ -> tsh_schedule_view ())]; *)
