...
, application sammp, processor izard type=U */
...
#define NSEMS 6 /* number of semaphores */

#define Semaphore_Thread_y 5
#define input1_o__louve_y_empty 4
#define input1_o__louve_y_full 3
...
input2_o,U,2.,louve,izard,ourson
louve,izard 
...louve,izard ...
ourson
{ #define NB_SENDERS_ 
  ...
  int recvs[] = {sd[3]};
  reliable_send(recvs, (char*)input2_o, 1*sizeof(int));}
    Precede(input2_o__louve_y_empty);
    Succede(input1_o__louve_y_full);
     
 
  
input1_o,U,2.,louve,izard,ourson
louve,izard 
...louve,izard ...
ourson
{ #define NB_SENDERS_ 
  #define NB_RECVS_ nb_recvs
  int recvs[] = {sd[3]};
  reliable_send(recvs, (char*)input1_o, 1*sizeof(int));}
    Precede(input1_o__louve_y_empty);
    /* TCP : nothing worth to TCP reliable sync (add_o,U,louve,ourson,root) */
  }} /* end loop_2 */
