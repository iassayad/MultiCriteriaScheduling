...
, application sampp, processor ourson type=U */
...
#define NSEMS 11 /* number of semaphores */

#define Semaphore_Thread_y 10
#define Semaphore_Thread_x 9
#define Sub_o__ourson_x_empty 8
#define Sub_o__ourson_x_full 7
#define input1_o__ourson_y_empty 6
#define input1_o__ourson_y_full 5
#define input2_o__ourson_y_empty 4
#define input2_o__ourson_y_full 3
#define Mul_o__ourson_y_empty 2
#define Mul_o__ourson_y_full 1
...
#define MYID_y 1 /* my index on y bus */
...
#define BOOTID 0 /* boot processor index */
  ...
  Precede(Mul_o__ourson_y_empty);
  {int i; for(i=0; i<2; i++){ /* loop_2 */
    Succede(input1_o__ourson_y_empty);
    for(pp=(char*)input1_o, len=1*sizeof(int); len!=0; pp+=n, len-=n)
    n = read(sd[0], pp, len);
    if(brev[0]) brev4((int*)input1_o, 1);
    Precede(input1_o__ourson_y_full);
    Succede(input2_o__ourson_y_empty);
    for(pp=(char*)input2_o, len=1*sizeof(int); len!=0; pp+=n, len-=n)
    n = read(sd[0], pp, len);
    if(brev[0]) brev4((int*)input2_o, 1);
    Precede(input2_o__ourson_y_full);
    Succede(Mul_o__ourson_y_full);
    write(sd[0], (char*)Mul_o, 1*sizeof(int));
    Precede(Mul_o__ourson_y_empty);
  }} /* end loop_2 */
...
#define MYID_x -100 /* my index on x bus */
#define NHOSTS_x 3 /* number of procrs connected on x bus */
...
#define BREVKEY 0x01234567
  ...
  {int i; for(i=0; i<2; i++){ /* loop_4 */
    Succede(Sub_o__ourson_x_empty);
    for(pp=(char*)Sub_o, len=1*sizeof(int); len!=0; pp+=n, len-=n)
    n = read(sd[-100], pp, len);
    if(brev[-100]) brev4((int*)Sub_o, 1);
    Precede(Sub_o__ourson_x_full);
  }} /* end loop_4 */
  return 0;
} /* end of com_thread_x */
...
int main(int argc, char* argv[]) { /* for link with C runtime boot */
  ...
  Precede(input1_o__ourson_y_empty);
  Precede(input2_o__ourson_y_empty);
  Precede(Sub_o__ourson_x_empty);
  {int i; for(i=0; i<2; i++){ /* loop_6 */
    Succede(input1_o__ourson_y_full);
    Succede(input2_o__ourson_y_full);
    Add_o[0] = input2_o[0] + input1_o[0];
    Precede(input1_o__ourson_y_empty);
    Precede(input2_o__ourson_y_empty);
    Succede(Sub_o__ourson_x_full);
    Succede(Mul_o__ourson_y_empty);
    Mul_o[0] = Add_o[0] * Sub_o[0];
    Precede(Sub_o__ourson_x_empty);
    Precede(Mul_o__ourson_y_full);
  }} /* end loop_6 */
