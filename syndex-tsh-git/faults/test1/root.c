...
, application sampp, processor root type=U */
...
#define NSEMS 13 /* number of semaphores */

#define Semaphore_Thread_y 12
#define Semaphore_Thread_x 11
#define input2_o__root_x_empty 10
#define input2_o__root_x_full 9
#define input1_o__root_x_empty 8
#define input1_o__root_x_full 7
#define input2_o__root_y_empty 6
#define input2_o__root_y_full 5
#define Mul_o__root_y_empty 4
#define Mul_o__root_y_full 3
#define input1_o__root_y_empty 2
#define input1_o__root_y_full 1
...
test -x $H.mnt||ln -s /bin/false $H.mnt);./$H.mnt'",
  ...
  {int i; for(i=0; i<2; i++){ /* loop_2 */
    ...
    Precede(input2_o__root_y_empty);
    Succede(Mul_o__root_y_empty);
    for(pp=(char*)Mul_o, len=1*sizeof(int); len!=0; pp+=n, len-=n)
    n = read(sd[1], pp, len);
    if(brev[1]) brev4((int*)Mul_o, 1);
    Precede(Mul_o__root_y_full);
  }} /* end loop_2 */
...
#define MYID_x 0 /* my index on x bus */
...
#define BREVKEY 0x01234567
  ...
  struct hostpnx {
    ...
    /* IamRoot: I start all other workstations */
  static struct hostpnx host[NHOSTS_x] = {  {0,"root","root"},
    {0,"izard","izard"}
  };
  char command[200];
  char CWD[80]; /* HOME-relative Current Working Directory */
  ...
  host[MYID_x].port = saddr.sin_port;
  gethostname(host[MYID_x].name, sizeof(host[MYID_x].name));
  for(i=0; i<NHOSTS_x; i++) if(i!=MYID_x) { /* start each other workstation */
    sprintf(command, "sh -c '\
H=%s;test -x $H.mnt||(rsh $H ln -s /bin/true ./%s/$H.mnt;\
test -x $H.mnt||ln -s /bin/false $H.mnt);./$H.mnt'",
            host[i].name, CWD);
    ...
    /* then rexec executable in same directory, with root name and port */
    sprintf(command+strlen(command), "cd %s\\;./%s %s %d",
            CWD, host[i].exec, host[MYID_x].name, ntohs(host[MYID_x].port));
    INFO(("%s\n", command)); /* trace the command line */
    if(fork()==0) return system(command); /* execute it */
    len = sizeof(saddr); /* wait for connexion: */
    ...
    read(sd[i], (char*)(brev+i), 4); /* receive byte-order */
    /* send all listening ports already collected */
    for(j=0; j<i; j++) if(j!=MYID_x) write(sd[i], &host[j], sizeof(host[j]));
    read(sd[i], (char*)&host[i].port, 2); /* receive listening port */
    INFO((">>> %s %d\n", host[i].name, ntohs(host[i].port)));
  }
  /* broadcast byte-orders */
  for(i=0; i<NHOSTS_x; i++) if(i!=MYID_x) write(sd[i], (char*)brev, sizeof(brev));
  ...
  Precede(input1_o__root_x_empty);
  Precede(input2_o__root_x_empty);
  {int i; for(i=0; i<2; i++){ /* loop_4 */
    Succede(input1_o__root_x_full);
    write(sd[1], (char*)input1_o, 1*sizeof(int));
    Precede(input1_o__root_x_empty);
    Succede(input2_o__root_x_full);
    write(sd[1], (char*)input2_o, 1*sizeof(int));
    Precede(input2_o__root_x_empty);
  }} /* end loop_4 */
...
int main(int argc, char* argv[]) { /* for link with C runtime boot */
  ...
  if(fork()==0) return com_thread_x(argv);
    ...
    Precede(Mul_o__root_y_empty);
  {int i; for(i=0; i<2; i++){ /* loop_6 */
    Succede(input1_o__root_x_empty);
    Succede(input1_o__root_y_empty);
    printf("input1_o[%d]: \n",0);;scanf("%d",&input1_o[0]);printf("%s\n","\n");;
    Precede(input1_o__root_x_full);
    Precede(input1_o__root_y_full);
    Succede(input2_o__root_x_empty);
    Succede(input2_o__root_y_empty);
    printf("input2_o[%d]: \n",0);;scanf("%d",&input2_o[0]);printf("%s\n","\n");;
    Precede(input2_o__root_x_full);
    Precede(input2_o__root_y_full);
    Succede(Mul_o__root_y_full);
    printf("Mul_o[%d] : %d\n",0,Mul_o[0]);;printf("%s\n","\n");;
    Precede(Mul_o__root_y_empty);
  }} /* end loop_6 */
