  ...
  let routes o1 o2 exclusives_routes =
	...
	| false ->
            ...
            List.iter (fun (og1,(opr2,x) as lnk1) ->
                           (*ps (name_of_operator opr2);*)  
                           List.iter (fun (og11,(opr22,_) as lnk2) ->
                                        (*ps (" ....  "^(name_of_operator opr22));*)
  ...
  let best_comb,best_freq,best_sp,best_esfs,best_delta,best_rel, best_ILambda, best_pui = 
    Hashtbl.fold  (fun _ (try_oprs_comb,try_freqs_comb) (tmp_comb,tmp_freq,tmp_sp,tmp_esfs,tmp_delta,tmp_rel,tmp_ILambda,tmp_pui) ->
	  ...
	  | false ->  (* pour une opn qui n'est pas un rt opn c'est tjrs le cas car new_comb = try_oprs_comb 
	      ...
	      (*debug_ps 0(*!yyyy*) debug_level (List.fold_left (fun s opr -> s^" "^(name_of_operator opr))
		...
		List.fold_left (fun delta_tmp opr ->
	      ...
	      let new_len  =  max !old_len (_esfs +. _delta) in
              ...
              let freq_max = freqs_oprs.(List.nth freqs_oprs ((List.length freqs_opr) - 1)) in
	      let number_of_processors_not_runing_this_operation = 
                          (List.length all_operators) - (List.length new_comb) in 
	      let added_power_for_processors_not_runing_this_operation = 
                           number_of_processors_not_runing_this_operation *. (new_len -. old_len) *. freq_max *. freq_max in
              let opns_power_tild = List.fold_left (
                                      fun power_tild (esfs,delta,freq) -> 
                                            let added_power_for_this_operation = 
					         match (esfs+.delta < new_len) with
                                                     | true ->
                                                             ((new_len -.(esfs +. delta))*.freq_max *. freq_max) + 
					             | false ->
                                                              0.
						  in								
					          match (esfs < old_len) with
                                                     | true ->
                                                              power_tild +. (delta - (old_len-esfs)*. freq *. freq) +. added_power_for_this_operation 
                                                                         +. added_power_for_processors_not_runing_this_operation
                                                     | false -> 
                                                              power_tild +. (delta *. freq *. freq) +. added_power_for_this_operation 
                                                                         +. added_power_for_processors_not_runing_this_operation
	     ...
	     (** issue 5 patch starts here **)             
             (match (((exact_ILambda <= !obj_big_lambda)&&(opns_power_tild <= !obj_pui))||(only_length)) with                 
		 ...
		 | false ->  (* in this test we have the same pressure as for the preceding proc comb, so we chose the qolution
                                using less procs *)
		     (match (((new_sp = tmp_sp)&&((List.length new_comb)<=(List.length tmp_comb))&&(not only_length))||
     		     (((exact_ILambda <= tmp_ILambda)&&(current_pui <= tmp_pui))&&(only_length)))  with (* warning 1 : the part after || of this test never holds here *)
		     | true  -> (*ps "*********************************";
			        debug_ps 0 debug_level (List.fold_left (fun s frq -> s^" "^(string_of_float frq))
				    ("................old freqs  = ") tmp_freq);
			        debug_ps 0 debug_level (List.fold_left (fun s frq -> s^" "^(string_of_float frq))
...
let best_oprs_comb opns graph random_media = 
  ...
  let best_comb, best_freq, best_comp,best_esfs,best_delta,best_rel,best_ILambda,best_pui = 
    ...
    |  _  -> best_comb, best_freq, best_comp,best_esfs,best_delta,best_rel,best_ILambda,best_pui) in  
  ...
  let s = Hashtbl.fold  
    ...
    for i = 1 to combinaisons do
      ...
      then begin
		 ...
		 begin                    
                   let ind = ref (-1) in  
                    (* issue 1 : there must be 4**k freq sets freqs_combs for each processors combinaison not only one set as done below *)
