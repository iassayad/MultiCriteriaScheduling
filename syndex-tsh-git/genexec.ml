...
let gen_communication_macro_name comclass =
...
let gen_macro_name graph operation operator offset phase =
  let opn =
    match operation.t_opn_class with
    | Communication comclass -> gen_communication_macro_name comclass
...
let suc_pre operator opn opndpd sucpre fullempty offset =
  ...
  offset^sucpre^ram^wait_type^"_("^sem_name^thread_name^")\n"
...
(*  ps ("PREDS of "^(identifier_of_operation operation));
   List.iter (fun pred -> ps (string_of_dpd pred)) preds; *)
  preds
...
let gen_loop_seq graph operator opns_on_operator chrono chrono_counter =    
  ...
  let opns_on_operator =
    List.map
      (fun opn ->
	(opn,opn.t_opn_condition,(preds_for_sems opn),(succs_for_sems opn)))
      opns_on_operator in
...
let included_libs () =
  let libraries = "syndex"::"fault_tolerance"::(Application.included_libs ()) in
...
(** Adds a Sync operation for operation opn in graph *)
let sync opn portout sender receivers executor graph =
  let rank = opn.t_opn_rank+1 in
  let data = data_of_communication opn in
  let comclass = Sync (data,sender,receivers,executor) in
  let recv_name = name_of_comclass comclass in
  let portin = {portout with t_prt_name="synchro_in";t_prt_dir=Port.In} in
...
let synchronize_sammp_broadcast sends graph archilib archiname =
  ...
  let synchronize_sammp_broadcast_send send =
    ...
    | Communication (Send (data,opr,oprs)) ->
	...
	let comclass = Send (data,opr,oprs) in
	...
	List.iter
	  (fun opr -> sync send portout operator recvs_operators opr graph)
...
(** [gen_operators_architecture oprs opr_main] returns the macros
   ...
   main operator [opr_main] *) let gen_operators_architecture oprs
   opr_main = let gen_operator (name,typelib,typename,_) = let
   _,_,gates,_,_n,_ = Architecture.operator_definitionname_content
   typelib typename in let gates = string_of_string_list (List.fold_left
   (fun l (gtype,gname) -> l@[gtype;gname]) [] gates) "," in
   processor_macro^"("^typename^","^name^","^gates^")\n" in let
   opr_main,opr_others = List.partition (fun (opr,_,_,_) ->
   opr=opr_main) oprs in let oprs = opr_main@(List.sort compare
   opr_others) in List.fold_left (fun s opr -> s^(gen_operator opr)) ""
   oprs
