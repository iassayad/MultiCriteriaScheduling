...
(********************************************************************) 
and simulation_ccr () = 
    let what_sim = "min-Length-respect-FailureRate" in
    let dir = ("/scratch/kalla/Benchmarks/Results/"^what_sim^"/")  in  
    ...
    output_string out_readme ("\n\n length, reliability and failure rate :\n\n");  
    output_string out_readme (" p = 4  \n ccr = [10.0] \n N = 10:50 (x100graphs) \n  rep max = 2 (donc 3)");  
    close_out out_readme;   
    let p = ref 0 in  
    let tab_p = [|4|] in  (* NUMBER OF PROCESSORS *) 
    for ind_p = 0 to 0 do  
      p := tab_p.(ind_p);        
      ...
      for ind_ccr = 1 to 2 do  
	ccr := tab_ccr.(ind_ccr);      
	(* file for mean replication rate of each graph *)
      	let aaaf_rep = (dir^"rep_aaaf_ccr-"^(string_of_float !ccr)^"_p-4") in   
        let out_aaaf_rep = open_out aaaf_rep in     
	(* files for schedule length *)
 	let aaa_len = (dir^"len_aaa_ccr-"^(string_of_float !ccr)^"_p-4") in   
        let aaaf_len  = (dir^"len_aaaf_ccr-"^(string_of_float !ccr)^"_p-4") in  
        let out_aaaf_len = open_out aaaf_len in    
        let out_aaa_len = open_out aaa_len in     
	(* files for schedule reliability *)
        let aaaf_rel  = (dir^"rel_aaaf_ccr-"^(string_of_float !ccr)^"_p-4") in 
        let aaa_rel   = (dir^"rel_aaa_ccr-"^(string_of_float !ccr)^"_p-4") in 
        let out_aaaf_rel = open_out aaaf_rel in    
        let out_aaa_rel = open_out aaa_rel in    
	(* algorithm graphs size  *) 
	for i = 1 to 5 do  
 	  let size =  i * 10 in
	  (* file for mean replication rate of each operation of all redundant graphs *)
      	  let aaaf_rep_process = (dir^"rep_aaaf_opn-"^(string_of_int size)^"_ccr-"^(string_of_float !ccr)^"_p-4") in   
	  ...
	  output_string out_aaa_rel (string_of_int size);output_string out_aaaf_rel (string_of_int size);  
          let dir_opr=(dir_graphs^"Random_graphs/p"^(string_of_int !p)^"/t"^(string_of_int size)^"/")in               
          (* 100 random algorithm graphs with the same size = i*10 *)  
	  ...
	  let nb_red = 50 in
	    ...
	    let name_rel = (dir_opr^algo_name^"_ccr"^(string_of_float !ccr)^"_"^archi_name^".rel") in
	    let  name_rel =   "/home/popart/kalla/SynDEx/SynDEx-work/procs.rel" in
            pss name;
	    open_file_name_simulation name;               
            ...
            output_string out_aaa_rel  (" "^(string_of_float !Latency_adequation.schedule_reliability));  
	    let len = !Latency_adequation.schedule_length in
	    let rel = !Latency_adequation.schedule_reliability in
	    let lambda = !Latency_adequation.schedule_big_lambda in
	    pss ("\n AAA : \n      L = "^(string_of_float  len)^"\n      R = "^(string_of_float  rel)^"\n     /\\ = "^(string_of_float  lambda)); ps "";
            (*** AAA-F ***) 
	    let rep = 3 in
	    adequation No_Flatten (Reliability_adequation.adequation len lambda 45. rep false name_rel); 
            output_string out_aaaf_len  (" "^(string_of_float !Reliability_adequation.schedule_length)); 
            output_string out_aaaf_rel  (" "^(string_of_float !Reliability_adequation.schedule_reliability)); 
            output_string out_aaaf_rep  (" "^(string_of_float !Reliability_adequation.rate_replication)); 	    
	    pss ("\n AAA-F ("^(string_of_float !Reliability_adequation.theta)^","^(string_of_int rep)
                 ^")\n      L = "^(string_of_float  !Reliability_adequation.schedule_length)
                  ...
                  ^"\n      Rep Rate = "^(string_of_float  !Reliability_adequation.rate_replication)^"% \n\n");
	    let index = ref 0 in
	    List.iter (fun rep -> median_replication.(!index) <-  rep::median_replication.(!index);index := !index+1;
		      ) !Reliability_adequation.replication_process;ps "";
            (*** write results to files ***)  
            flush out_aaa_len; flush out_aaaf_len;
	    flush out_aaa_rel; flush out_aaaf_rel;
            flush out_aaaf_rep;
            ignore(close_file ());
          done;  
	  ...
	  output_string out_aaa_rel "\n"; output_string out_aaaf_rel "\n";
	  output_string out_aaaf_rep "\n"; 
	done;    
	close_out out_aaa_len;close_out out_aaaf_len;
        close_out out_aaa_rel;close_out out_aaaf_rel;
	close_out out_aaaf_rep;
      done;
    done
...
| false -> let name,name_rel = 
              match true with    (*/scratch/kalla/Benchmarks/Random_graphs/p4/t30/alg30_red10_ccr1._arc4.sdx*)
	      | true ->  "/scratch/kalla/Benchmarks/Random_graphs/p4/t30/alg30_red10_ccr1._arc4.sdx",
 		         "/home/popart/kalla/SynDEx/SynDEx-work/procs.rel";
	      | false -> "/home/popart/kalla/SynDEx/SynDEx-work/sampp.sdx",  
                         "/home/popart/kalla/SynDEx/SynDEx-work/procs.rel" in
  open_file_name_simulation name;                                 
  (**********************************) 
  (************    AAA     **********) 
  adequation No_Flatten (Latency_adequation.adequation name_rel);        
  ...
  (*schedule_view Reliability_adequation.pretty_conv_graph "AAA";*)
  (************************************)
  (************    AAA-F     **********)
<<<<<<< ihm_ctk.ml
  let r = 2 in
  adequation No_Flatten (Reliability_adequation.adequation  !Latency_adequation.schedule_length !Latency_adequation.schedule_big_lambda 45. r name_rel);
  pss ("\n AAA-F ("^(string_of_float !Reliability_adequation.theta)^","^(string_of_int r)
              ^")\n      L = "^(string_of_float  !Reliability_adequation.schedule_length)
              ^"\n      R = "^(string_of_float  !Reliability_adequation.schedule_reliability)
              ^"\n     /\\ = "^(string_of_float  !Reliability_adequation.schedule_big_lambda)
       ^"\n      Rep Rate = "^(string_of_float  !Reliability_adequation.rate_replication)^"% \n\n"); 
  (*schedule_view Reliability_adequation.pretty_conv_graph "AAA Reliability";   *)
  (************************************)
=======
  let r = 1 in     
  ...
  adequation No_Flatten (Reliability_adequation.adequation schedule_length schedule_big_lambda 45. r true name_rel);
  ...
  (*let schedule_length = !Latency_adequation.schedule_big_lambda  (*!Reliability_adequation.schedule_length*)           in
  let schedule_big_lambda = Adequationtypes.get_lambda  !Reliability_adequation.schedule_reliability !Reliability_adequation.schedule_length  in
  ...
  (*********  PARETO GRAPH  *********
  let res_lambda = ref "/\\  = " in
  let res_len = ref "Len = " in 
  let res_rel = ref "Rel = " in
  let res_rep = ref "Rep =" in 
  for i= 0 to 8 do 
    let lam = [|1.0; 0.5; 0.1; 0.05; 0.01; 0.005; 0.001|] in 
    let lam = [|0.00166831397849;   0.00125215619272;    0.00100000000000000;  0.000777105617552;  0.000166831397849; 0.0000834156989247;  
		0.0000166831397849; 0.00000834156989247; 0.00000166831397849|] in 
    let r = 3 in 
    let new_lambda = lam.(i) (* *. !Latency_adequation.schedule_big_lambda*) in 
    ps ("try lambda = "^(string_of_float new_lambda ));
    adequation No_Flatten (Reliability_adequation.adequation  !Latency_adequation.schedule_length new_lambda 45. r name_rel);  
    res_lambda := (!res_lambda^"    "^(string_of_float !Reliability_adequation.schedule_big_lambda)); 
