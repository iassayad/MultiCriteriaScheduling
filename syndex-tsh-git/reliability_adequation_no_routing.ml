...
| hd::tl -> 
  ...
  let routes o1 o2 exclusives_routes =
	...
	| false ->
	    ...
	    let subroutes = List.map (function (og1,(opr2,_)) as lnk -> lnk,(r opr2 o2 (og1::route))) links in
...
let compromise _ILen _IRel _ILambda =
  ...
  let rec create_copies operation graph count =   
    match count with
      | 0 -> []
      | _ -> 
	  let copie = (create_copie operation graph true count) in
  ...
  let best_comb, best_sp,best_esfs,best_delta,best_rel, best_ILambda = 
    Hashtbl.fold  (fun _ try_oprs_comb (tmp_comb,tmp_sp,tmp_esfs,tmp_delta,tmp_rel,tmp_ILambda) ->
			 ...
			 | false ->
			     ...
			     debug_ps 10 debug_level (List.fold_left (fun s opr -> s^" "^(name_of_operator opr)) ("\n   try on operators  = ") new_comb); 
...
(*******************************)
  let adequation  _obj_len _obj_big_lambda _theta _level_max fix_level _reliability_file_name graph =       
    ...
    (*List.iter (fun (opn,oprs_id) -> ps (opn^" "^(string_of_int (List.length oprs_id)))) !scheduled_opns;*)
    ...
    let schedulables = ref (List.map (fun (id_opn,oprs_id) -> 
					let opn = Hashtbl.find graph id_opn in
					let red = (List.length oprs_id)-1 in
					opn::(create_copies opn graph red),oprs_id 
				     ) !scheduled_opns) in
    ...
    try        
      let size = List.length !schedulables in
      for i=1 to size do
        debug_ps !xxxx debug_level  "\n ========================================================";
	...
	let opns_opts,oprs_id = List.nth !schedulables (i-1) in
	let oprs_opts = List.map (fun opr_id -> List.find (fun opr -> (name_of_operator opr)=opr_id) all_operators) oprs_id in
	...
	(*ps (List.fold_left (fun s (opn,opr) -> s^" "^(name_of_operation opn)^" "^(name_of_operator opr)) "\n to schedule = " opts);*)
	...
	debug_ps !xxxx debug_level ((List.fold_left (fun s opn -> s^" "^(name_of_operation opn)) ("\n end schedule  = ")  [(List.hd opns_opts)])
	...
	if not (is_routing_operation (List.hd opns_opts)) then reps := !reps^" ;  "^(string_of_int (List.length (opns_opts)));
