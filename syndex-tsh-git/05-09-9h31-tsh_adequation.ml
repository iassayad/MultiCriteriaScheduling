...
let compute_rel_with_com all_opns scheduled_opns random_medias =   
  ...
  for i=1 to !scheduled_opns do			  
    ...
    debug_ps !xx debug_level ((name_of_operator (pi next_opn))
                            ^"\n  lambda =  "^(string_of_float lambda_opr)
  ...
  debug_ps !xx debug_level ((name_of_operation (List.hd all_opns))^","^(name_of_operator (pi (List.hd all_opns)))^" = "^(string_of_float (1. -. !non_rel)));
  (1. -. !non_rel)
...
(*if (is_routing_operation (List.hd opns)) then xxxx := 1 else xxxx := 0;*)
  let best_comb, best_sp,best_esfs,best_delta,best_rel, best_ILambda = 
    Hashtbl.fold  (fun _ try_oprs_comb (tmp_comb,tmp_sp,tmp_esfs,tmp_delta,tmp_rel,tmp_ILambda) ->
			 ...
			 | false ->			     
			     debug_ps !yyyy debug_level (List.fold_left (fun s opr -> s^" "^(name_of_operator opr)) ("................>  try on operators  = ") new_comb); 
			     ...
			     let sum_Ci   = !deltas_sum +. delta_exe_sum  +. delta_com_sum in
                             debug_ps !xx debug_level (" Sum = "^(string_of_float sum_Ci));   			     
			     let exact_ILambda = get_lambda new_rel sum_Ci in	    		
                             debug_ps !xx debug_level (" lam = "^(string_of_float exact_ILambda));   
			     ...
			     deschedule_opns new_comb opns graph;
                               ...
                               let res = ("                    pression="^(string_of_float new_sp)^", L="^(string_of_float new_len)
                                 ...
                                 debug_ps 0 debug_level  (ppp^res);
...
let best_oprs_comb opns graph random_media = 
  ...
  debug_ps (*!xxxx*)0 debug_level ((List.fold_left (fun s opr -> s^" "^(name_of_operator opr)) ((name_of_operation (List.hd opns))^"\n oprs bests = ") best_comb)
...
(*******************************)
  let adequation  _obj_len _obj_big_lambda _theta _level_max fix_level _reliability_file_name _frequency_file_name graph =       
    let s = Hashtbl.fold  
      (fun _ opn s -> 
    ...
    let freq = [|1;2;3|] in
    Hashtbl.iter (fun i oprs_comb -> 
                    ps (List.fold_left (fun s opr -> s^" "^(name_of_operator opr)) ("................>  try on operators  = ") oprs_comb);
                    let len = (List.length oprs_comb) in
                    let first_com = (List.hd oprs_comb) in ()
	...
	debug_ps 10 debug_level (List.fold_left (fun s opn -> s^"   "^(name_of_operation opn)) ("\n \n schedule  : ")  opns_opts);  
        ...
        debug_ps !xx debug_level ((List.fold_left (fun s opr -> s^" "^(name_of_operator opr)) ("\n\n................................................................\n\n"^(name_of_operation (List.hd opns_opts))^"\n oprs bests = ") oprs_opts)^" .....  L =  "^(string_of_float comp_opt)^"  /\\ =   "^(string_of_float  lambda_opt));
