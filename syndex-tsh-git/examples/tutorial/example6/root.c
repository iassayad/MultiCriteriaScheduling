...
, application example6, processor root type=U */
...
#define In_o__root_y_empty 6
#define In_o__root_y_full 5
#define mul_o__root_y_empty 4
#define mul_o__root_y_full 3
#define add_o__root_y_empty 2
#define add_o__root_y_full 1
...
test -x $H.mnt||ln -s /bin/false $H.mnt);./$H.mnt'",
  ...
  Precede(In_o__root_y_empty);
  {int i; for(i=0; i<5; i++){ /* loop_2 */
    Succede(In_o__root_y_full);
    write(sd[1], (char*)In_o, 1*sizeof(int));
    Precede(In_o__root_y_empty);
...
int main(int argc, char* argv[]) { /* for link with C runtime boot */
  ...
  {int i; for(i=0; i<5; i++){ /* loop_4 */
    Succede(In_o__root_y_empty);
    printf("In_o[%d]: \n",0);;scanf("%d",&In_o[0]);printf("%s\n","\n");;
    Precede(In_o__root_y_full);
