...
, application example6, processor pc1 type=U */
...
#define mul_o__pc1_y_empty 6
#define mul_o__pc1_y_full 5
#define add_o__pc1_y_empty 4
#define add_o__pc1_y_full 3
#define In_o__pc1_y_empty 2
#define In_o__pc1_y_full 1
...
int _In_o[1];
#define In_o sp->_In_o
int _add_o[1];
#define add_o sp->_add_o
int _conv_o[1];
#define conv_o sp->_conv_o
int _mul_o[1];
#define mul_o sp->_mul_o
int _cste2_pc1_o[1];
#define cste2_pc1_o sp->_cste2_pc1_o
...
#define BOOTID 0 /* boot processor index */
  ...
  Precede(add_o__pc1_y_empty);
  Precede(mul_o__pc1_y_empty);
  {int i; for(i=0; i<5; i++){ /* loop_2 */
    Succede(In_o__pc1_y_empty);
    for(pp=(char*)In_o, len=1*sizeof(int); len!=0; pp+=n, len-=n)
    n = read(sd[0], pp, len);
    if(brev[0]) brev4((int*)In_o, 1);
    Precede(In_o__pc1_y_full);
    Succede(add_o__pc1_y_full);
    write(sd[0], (char*)add_o, 1*sizeof(int));
    Precede(add_o__pc1_y_empty);
    Succede(mul_o__pc1_y_full);
...
int main(int argc, char* argv[]) { /* for link with C runtime boot */
  ...
  cste2_pc1_o[0]=2;
  Precede(In_o__pc1_y_empty);
  {int i; for(i=0; i<5; i++){ /* loop_4 */
    conv_o[0] = cste2_pc1_o[0] + 1;
    Succede(In_o__pc1_y_full);
    Succede(add_o__pc1_y_empty);
    add_o[0] = In_o[0] + cste2_pc1_o[0];
    Precede(add_o__pc1_y_full);
    Succede(mul_o__pc1_y_empty);
    mul_o[0] = In_o[0] * conv_o[0];
    Precede(In_o__pc1_y_empty);
    Precede(mul_o__pc1_y_full);
  }} /* end loop_4 */
